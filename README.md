...
| Version | Notes |
|---|---|
| v2.0.0 | **Phase 1 release.** NEW: saas-dev auto-router skill (single entry point). NEW: django-auth-dev — Pattern C multi-user auth, separate AbstractBaseUser per type, UserTypeAuthMiddleware, JWT per type, RBAC, token revocation. NEW: service layer pattern (cross-app logic). NEW: sequential code generation with select_for_update(). MCP auto-detection. Session context management across skills. |
| v1.5.2 | **Stability release.** 11 fixes from stress test: Phase 0 order fixed in all 3 skills, duplicate F1.2/index.ts removed, frontend question removed from backend skill, migration command shown after every model task, Quick Change Plan for single-field tasks, proactive bulk_create warning, TableSkeleton enforced, new project setup sequence, FilterSet update auto-suggested. |
